...
import { Stack, Box, Container, Grid, SxProps, Typography, Link } from "@mui/material";
...
const getSvgIcon = (Icon: string) => {
    return (
        // <SvgIcon>
        <Box component={"img"} sx={{ px: 1, py: 0 }} src={Icon} height={60} width={60}></Box>
        // </SvgIcon>
    );
};
...
const Footer = () => {
    ...
    const pageListing = (
        <Stack spacing={2} sx={{ flex: "1", pl: 2, width: "100%" }}>
            {pages.map((page) => (
                <div>
                    <Typography component={Link} sx={bold_no_underline} href={page.relativePath}>
                        {page.name}
                    </Typography>
                </div>
            ))}
